...
# ----------------------------------------------------------
# install_rust: 安装 Rust 工具链（rsproxy 镜像加速）
...
install_rust() {
    ...
    # PATH 不会在两次执行之间保留：必须先加载已安装的 cargo env 再检查，
    # 否则新 shell 中 command -v rustup 永远失败，导致每次都重装 rustup
    if [ -f "$HOME/.cargo/env" ]; then
        source "$HOME/.cargo/env"
    fi

    if ! command -v rustup &>/dev/null; then
        echo ">>> Installing rustup (rsproxy mirror)..."
        retry 3 curl --proto '=https' --tlsv1.2 -sSf -o /tmp/rustup-init.sh https://rsproxy.cn/rustup-init.sh
    ...
    # 配置 crates.io 国内镜像（rsproxy），避免 cargo build 下载依赖超时
    mkdir -p "$HOME/.cargo"
...
EOF
    echo ">>> Cargo registry mirror: rsproxy.cn"
...
# ----------------------------------------------------------
install_maturin() {
    # pip 用户级安装的可执行文件在 ~/.local/bin，非交互 shell 的默认 PATH 不含该目录；
    # 已在 PATH 中则跳过，保证幂等
    case ":${PATH}:" in
        *:"$HOME/.local/bin":*) ;;
